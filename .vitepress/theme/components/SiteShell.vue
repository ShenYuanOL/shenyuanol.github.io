<script setup lang="ts">
import { useData, withBase } from 'vitepress';
import { Icon } from '@iconify/vue';
import { computed, ref } from 'vue';
import SiteLightbox from './SiteLightbox.vue';

const { site, theme } = useData();

const hasDevDocs = computed(() => theme.value?.hasDevDocs === true);

const scrolled = ref(false);

function onScroll() {
  scrolled.value = window.scrollY > 8;
}

function applyTheme(theme: 'light' | 'dark') {
  document.documentElement.classList.toggle('dark', theme === 'dark');
}

function toggleTheme() {
  const dark = document.documentElement.classList.contains('dark');
  applyTheme(dark ? 'light' : 'dark');
  try {
    localStorage.setItem('xiao-theme', dark ? 'light' : 'dark');
  } catch {
    /* 忽略隐私模式异常 */
  }
}

function initTheme() {
  try {
    const saved = localStorage.getItem('xiao-theme');
    if (saved === 'light' || saved === 'dark') {
      applyTheme(saved);
      return;
    }
  } catch {
    /* 忽略隐私模式异常 */
  }
  applyTheme(window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
}

if (typeof window !== 'undefined') {
  initTheme();
  onScroll();
  window.addEventListener('scroll', onScroll, { passive: true });
}
</script>

<template>
  <div class="x-site">
    <header class="x-header">
      <div class="x-header-inner" :class="{ 'x-header-scrolled': scrolled }">
        <a :href="withBase('/')" class="x-site-name">{{ site.title }}</a>
        <nav class="x-nav">
          <a :href="withBase('/archives')">归档</a>
          <a :href="withBase('/tags')">标签</a>
          <a :href="withBase('/categories')">分类</a>
          <a v-if="hasDevDocs" :href="withBase('/dev')">文档</a>
          <a :href="withBase('/about')">关于</a>
          <button class="x-theme-toggle" type="button" aria-label="切换主题" @click="toggleTheme">
            <Icon icon="mdi:white-balance-sunny" class="x-icon-sun" />
            <Icon icon="mdi:moon-full" class="x-icon-moon" />
          </button>
        </nav>
      </div>
    </header>

    <main class="x-main">
      <div class="x-home-bgimg"></div>
      <div class="x-shell">
        <slot />
      </div>
    </main>

    <footer class="x-footer">
      <div class="x-footer-shell">
        <hr class="x-footer-divider" />
        <div class="x-footer-inner">
          <span>© {{ new Date().getFullYear() }} {{ site.title }}</span>
          <span>Powered by 晓</span>
        </div>
      </div>
    </footer>

    <SiteLightbox />
  </div>
</template>

<style scoped>
.x-icon-sun {
  display: none;
}

html.dark .x-icon-sun {
  display: inline;
}

html.dark .x-icon-moon {
  display: none;
}

.x-home-bgimg {
  position: relative;
  height: 600px;
  margin-top: -90px;
  overflow: hidden;
  background-color: var(--page-bg);
  background-image: linear-gradient(to top,
      var(--page-bg),
      color-mix(in srgb, var(--page-bg) 70%, transparent) 36%,
      color-mix(in srgb, var(--page-bg) 18%, transparent) 68%,
      transparent 100%),
    url("../../../public/images/web/1.png");
  background-position: center center;
  background-size: cover;
  background-repeat: no-repeat;
  box-shadow: inset 0 -80px 80px color-mix(in srgb, var(--page-bg) 25%, transparent);
}

.x-home-bgimg::after {
  content: "";
  position: absolute;
  inset: 0;
  background: linear-gradient(to bottom,
      transparent 35%,
      color-mix(in srgb, var(--page-bg) 22%, transparent) 68%,
      var(--page-bg) 100%);
  pointer-events: none;
}
</style>
